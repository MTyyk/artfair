...
const pathDataVariants: PathData[] = [
...
// Hook — call once in the parent, pass the returned fn to each <DrawUnderline>
export function useDrawUnderlineVariants() {
  const indexRef = useRef<number | null>(null);
  return useCallback((): PathData => {
    if (indexRef.current === null) {
      indexRef.current = Math.floor(Math.random() * pathDataVariants.length);
    } else {
      indexRef.current = (indexRef.current + 1) % pathDataVariants.length;
    }
    return pathDataVariants[indexRef.current];
  }, []);
}
...
// Wrapper — drop around any <button> or <Link> to add the animated underline on hover
export const DrawUnderline: React.FC<DrawUnderlineProps> = ({ children, getSvgVariant, className }) => {
  ...
  useEffect(() => {
    ...
    enterTweenRef.current = gsap.to(path, {
      ...
      drawSVG: '100%',
      ease: 'power2.inOut',
      onComplete: () => { enterTweenRef.current = null; },
    });
  }, [currentSvg]);

  const handleMouseEnter = () => {
    if (enterTweenRef.current?.isActive()) return;
    setCurrentSvg(getSvgVariant());
  };

  const handleMouseLeave = () => {
    const path = pathRef.current;
    if (!path) { setCurrentSvg(null); return; }
    ...
    const startLeave = () => {
      ...
      leaveTweenRef.current = gsap.to(path, {
        ...
        drawSVG: '100% 100%',
        ease: 'power2.inOut',
        onComplete: () => { leaveTweenRef.current = null; setCurrentSvg(null); },
      });
    };

    if (enterTweenRef.current?.isActive()) {
      enterTweenRef.current.eventCallback('onComplete', startLeave);
    } else {
      startLeave();
    }
  };
  ...
  return (
    ...
    >
      {children}
      <span className="absolute inset-x-0 bottom-[-6px] h-[14px] pointer-events-none overflow-visible">
        {currentSvg && (
          <svg
